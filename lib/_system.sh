...
system_create_user() {
  ...
  sudo su - root <<EOF
  # Verificar se o usuário deploy já existe
  if id "deploy" &>/dev/null; then
    echo "Usuário deploy já existe. Removendo..."
    userdel -r deploy 2>/dev/null || true
  fi
  
  # Verificar se o grupo sudo existe, se não, usar wheel (alternativa)
  ...
  # Criar usuário com método mais seguro
  useradd -m -s /bin/bash -G \${SUDO_GROUP} deploy
  
  # Definir senha usando chpasswd (método mais seguro)
  echo "deploy:${mysql_root_password}" | chpasswd
  
  # Garantir que o usuário tenha acesso sudo
